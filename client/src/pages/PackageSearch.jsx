...
export default function PackageSearch() {
  ...
  const personsFor = (id) => personsById[id] ?? 1;
  const setPersonsFor = (id, v) => setPersonsById({ ...personsById, [id]: v });
  ...
  async function handleSearch(e) {
    ...
    try {
      ...
      const { data } = await axiosClient.get('/packages/search', { params });
  ...
  async function handleBook(pkg) {
    ...
    try {
      ...
      const { data } = await axiosClient.post('/payments/packages/checkout', {
        packageId: pkg.id,
        persons: personsFor(pkg.id),
      ...
      setResults((prev) =>
        prev.map((p) =>
          p.id === pkg.id ? { ...p, available_slots: p.available_slots - personsFor(pkg.id) } : p
        )
      );

      const basePrice = pkg.price_per_person * personsFor(pkg.id);
      ...
      const options = {
        ...
        handler: function () {
          const discountNote = discountApplied
            ...
            : '';
          setBookingSuccess(
            `Payment received for booking ${booking.id}.${discountNote} Confirming your booking — refresh in a few seconds to see it finalized.`
          );
        },
        modal: {
          ondismiss: function () {
      ...
      setBookingError(`${pkg.name}: ${msg}`);
    } finally {
      setBookingId(null);
    }
  }

  async function handleBookWithWallet(pkg) {
    if (!user) {
      navigate('/login');
      return;
    }

    setWalletBookingId(pkg.id);
    setBookingError(null);
    setBookingSuccess(null);

    try {
      const { data } = await axiosClient.post('/packages/pay-with-wallet', {
        packageId: pkg.id,
        persons: personsFor(pkg.id),
      });
      ...
      if (err.response?.status === 402) {
        ...
        setBookingError(
          `${pkg.name}: Insufficient wallet balance. You have ₹${Number(balance).toLocaleString('en-IN')}, need ₹${Number(required).toLocaleString('en-IN')}. Top up your wallet from your profile.`
        );
      } else {
    ...
    <>
      ...
      <section className="ts-hero" style={{ minHeight: 'auto', height: 'auto' }}>
        ...
        <div className="container ts-hero-content" style={{ padding: '1.5rem 0 1.25rem' }}>
          ...
          <p className="ts-hero-body mb-2" style={{ fontSize: '0.9rem' }}>
            Browse and book curated holiday packages across TravelSphere&apos;s network.
          </p>
          ...
          >
            <div className="col-md-8">
              <label className="ts-label" htmlFor="pkg-destination">Destination</label>
              <input
                id="pkg-destination"
                type="text"
                name="destination"
            ...
            <div className="col-md-4">
              <button type="submit" className="ts-btn-primary" disabled={searching}>
                {searching ? 'Searching...' : 'Search packages'}
              </button>
            </div>
          </form>
        </div>
      ...
      <div className="ts-page-bg">
        <div className="container pt-3 pb-5">
          ...
          {!user && (
            <Notice tone="info">
              You can search freely — <a href="/login" className="ts-switch-link">log in</a> to book a package.
            </Notice>
          )}

          {searched && (
            <div className="ts-section-title">
              {results.length > 0 ? 'Available packages' : 'No packages found'}
            </div>
          )}

          {searched && results.length === 0 && !searchError && (
            <div className="ts-empty-state">No packages found. Try a different destination.</div>
          )}
                ...
                return (
                  <div className="ts-row-item" key={pkg.id}>
                    <div className="row align-items-center g-3">
                      ...
                      <div className="col-md-3">
                        <div className="small text-muted">{pkg.description}</div>
                        <div className="small text-muted">
                          Departs {new Date(pkg.departure_date).toLocaleDateString()}
                        </div>
                      </div>
                      ...
                      <div className="col-md-4">
                        <div className="d-flex gap-2 mb-2">
                          <select
                            className="ts-input"
                            style={{ maxWidth: 100 }}
                            value={personsFor(pkg.id)}
                            onChange={(e) => setPersonsFor(pkg.id, Number(e.target.value))}
                            disabled={isFull}
                          >
                            {[1, 2, 3, 4].map((n) => (
                        ...
                        {!isFull && (
                          <input
                            ...
                            value={couponFor(pkg.id)}
                            onChange={(e) => setCouponFor(pkg.id, e.target.value.toUpperCase())}
